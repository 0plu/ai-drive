    ...
    <div class="login-content">
      ...
      <div class="auth-card">
        ...
        >
          ...
          <el-form-item prop="password" class="input-group">
            <div class="label-row">
        ...
        >
          ...
          <el-form-item prop="email" class="input-group">
...
<script setup lang="ts">
  ...
  const loginForm = reactive({ username: 'user2', password: 'user123456', challenge: '' })
  ...
  const toggleMode = () => {
    // 如果不允许注册，禁止切换到注册页面
    if (!allowRegister.value && !isFirstUse.value && activeTab.value === 'login') {
  ...
  const checkSysInfo = async () => {
    try {
      ...
      // 如果获取失败，默认允许注册（向后兼容）
      allowRegister.value = true
      console.log('[Login] checkSysInfo FAILED, error:', e, 'allowRegister set to true')
    }
  }
...
<style scoped>
  ...
  :deep(.input-group .el-form-item__content) {
    display: block;
  }
