...
export const DEFAULT_UPLOAD_CONFIG: UploadConfig = {
  chunkSize: UPLOAD_CONFIG.CHUNK_SIZE || 5 * 1024 * 1024, // 每个分片的大小，优先取全局配置，否则默认 5MB (5 * 1024 * 1024 字节)
  maxConcurrentChunks: 3, // 单个文件在上传时，允许同时发出请求的最大分片数量（默认同时传 3 个分片）
  maxFileSize: UPLOAD_CONFIG.MAX_FILE_SIZE || 10 * 1024 * 1024 * 1024, // 允许上传的最大文件大小，优先取全局配置，否则默认 10GB
  maxConcurrentFiles: 2 // 多个文件一起上传时，允许同时处于“上传中”状态的最大文件数量（默认 2 个）
}
...
export const calculateFileMD5 = (
  ...
  chunkSize: number, // 分片大小（字节），每次读取文件时最多读这么多
  onProgress?: (progress: number) => void // 可选的进度回调，参数为0~100的整数
): Promise<string> => {
  return new Promise((resolve, reject) => {
    // 创建增量MD5计算器，支持分批追加数据后统一输出哈希值
    const spark = new SparkMD5.ArrayBuffer()
    // 创建文件读取器，用于将文件片段读入内存
    const fileReader = new FileReader()
    // 当前已读取完成的分片序号（从0开始计数）
    let currentChunk = 0
    // 文件按 chunkSize 切割后的总分片数
    const chunks = Math.ceil(file.size / chunkSize)

    // 文件片段读取成功后的回调
    fileReader.onload = e => {
      if (e.target?.result) {
        // 将刚读取的分片数据追加到MD5计算器中（增量计算，不占用完整文件内存）
        spark.append(e.target.result as ArrayBuffer)
        // 已完成分片数+1
        currentChunk++

        // 如果调用方传了进度回调，计算当前进度百分比并通知
        if (onProgress) {
        ...
        if (currentChunk < chunks) {
          ...
          // 所有分片都已读取完毕，调用 spark.end() 输出最终的MD5哈希值
          resolve(spark.end())
        }
      } else {
        // 读取结果为空（理论上不应发生），视为读取失败
    ...
    // 读取下一个分片：计算当前分片的起止字节位置，切片后读入内存
    const loadNextChunk = () => {
      const start = currentChunk * chunkSize // 当前分片在文件中的起始字节
      const end = Math.min(start + chunkSize, file.size) // 结束字节，最后一片不能超出文件末尾
      const chunk = file.slice(start, end) // 利用 Blob.slice 切出当前分片（不复制数据，仅创建引用）
      fileReader.readAsArrayBuffer(chunk) // 异步将分片读入为 ArrayBuffer，完成后触发 onload
    }

    // 启动第一片读取，后续由 onload 回调驱动 loadNextChunk 形成读取链
    loadNextChunk()
  })
}
...
export const calculateChunkMD5 = (chunk: Blob): Promise<string> => {
  return new Promise((resolve, reject) => {
    // 创建增量MD5计算器，用于处理ArrayBuffer格式的数据
    const spark = new SparkMD5.ArrayBuffer()
    // 创建文件读取器，将Blob数据读入内存
    const fileReader = new FileReader()
    ...
    fileReader.onload = e => {
      if (e.target?.result) {
        // 将读取到的ArrayBuffer数据追加到MD5计算器
        spark.append(e.target.result as ArrayBuffer)
        // 单个分片只需一次读取，直接调用 end() 输出最终MD5哈希值
        resolve(spark.end())
      } else {
        // 读取结果为空，视为读取失败
    ...
    // 异步将整个分片Blob读入为ArrayBuffer，完成后触发 onload 回调
...
export const uploadSingleFile = async (params: UploadParams): Promise<ApiResponse<any> | void> => {
  ...
  try {
    ...
    // 向服务器发送文件的基本信息：整体 MD5、文件名、大小、所有分片 MD5。
    ...
    // 服务器返回三种可能
    // 200：文件已存在 → 秒传，直接标记上传成功，结束。
    // 201：文件需要上传，返回 precheck_id 和已存在的分片列表（用于断点续传）。
    // 其他：预检失败，报错终止。
